...
NAME = re.compile(r"^[a-z0-9][a-z0-9-]*\.(page|list|theme)\.json$")
# Section is not a published resource kind: its definition lives inline in the Page that uses
# it. A stray *.section.json would sail past NAME's kind-specific glob unnoticed, so it gets
# its own explicit rejection below instead.
STRAY_SECTION = re.compile(r"^[a-z0-9][a-z0-9-]*\.section\.json$")
...
def main() -> int:
    ...
    errors = 0
    for path in localized_paths("*.section.json"):
        if path.is_file() and STRAY_SECTION.fullmatch(path.name):
            fail(path, "standalone Section resources are no longer published; embed the Section in the Page that uses it")
            errors += 1
