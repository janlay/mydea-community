...
def main() -> int:
    ...
    for directory, kind in KINDS.items():
        paths = (sorted((ROOT / directory).glob("**/*")) if kind == "theme"
                 else sorted((ROOT / "collections").glob(f"*/*/*.{kind}.json")))
        for path in paths:
            if not path.is_file():
                continue
            language = None
            if kind != "theme":
                ...
                if path.parent.parent.parent != ROOT / "collections" or not LANGUAGE.fullmatch(language):
                    fail(path, "localized resources must be stored as collections/<id>/<language>/<file>")
                    errors += 1
    ...
    try:
        ...
        for category in KINDS:
            ...
            for identifier in entries:
                available = ({identifier for p in (ROOT / "collections").glob(f"*/*/*.{KINDS[category]}.json")
                             for identifier in (p.stem.rsplit(".", 1)[0], f"{p.parent.parent.name}/{p.stem.rsplit('.', 1)[0]}")}
