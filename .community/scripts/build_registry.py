...
HIDDEN_SERVICES = {"library", "mydea"}


def visible_service(value: object) -> str | None:
    return value if isinstance(value, str) and value and value not in HIDDEN_SERVICES else None
...
def build_catalog(directory: str, kind: str, language: str | None, featured: object, metadata: object) -> list[dict]:
    ...
    for path in sorted(paths):
        ...
        if tags:
            item["tags"] = tags
        if kind == "section":
            service = visible_service(data.get("service"))
            if service:
...
def build(args: argparse.Namespace) -> None:
    ...
    languages = sorted({path.parent.name for path in localized_paths("*.json")})
